...
import { foodAlergies } from "@/app/store";
import { Button } from "./ui/button";
import { Input } from "./ui/input";

export const AlergyForm = () => {
  const [next] = useStep((state) => [state.next]);
  const [option, setOptions] = React.useState(foodAlergies);
  const [input, setInput] = React.useState("");
  const [setAlergy] = useUserPreferences((state) => [state.setFoodAlergies]);
  console.log(input);
  const handleSubmit = () => {
